...
ggplot(df %>% group_by(Especie) %>% summarise(Cant = n()), 
       aes(x = reorder(Especie, Cant), y = Cant)) +
  geom_col(color = paleta[7],
                 fill = paleta[8]) +
  ...
  nombres <-
    paste(round((
      table(Origen) / sum(table(Origen)) * 100
    ), 2), "%", sep = "")
  nombres <-
  ...
  ggplot(
    ...
    geom_text(
      aes(label = round(InclinacionPromedio, 1)),
      vjust = 1.5,
      color = "white",
      size = 3.5
    ) +
  ...
  ggplot(
    ...
    geom_text(
      aes(label = round(numeroBrotes, 1)),
      hjust = 1.5,
      color = "white",
      size = 3.5
    ) +
  ...
  ggplot(
    ...
    geom_text(
      aes(label = round(BrotesPromedio, 1)),
      hjust = 1.5,
      color = "white",
      size = 3.5
    ) +
  ...
  ggplot(
    ...
    labs(x = 'Especie', y = 'Altura promedio (m)') +
    geom_text(
      aes(label = round(AlturaPromedio, 1)),
      hjust = 1.5,
      color = "white",
      size = 3.5
    ) +
